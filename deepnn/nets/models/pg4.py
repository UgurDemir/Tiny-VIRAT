...
class PG4(nn.Module):
    ...
    def __init__(self, nch=32, actenc='lrelu', actdec='lrelu', acto='tanh', norm='spec', conv_mode='3D'):
        super(PG4, self).__init__()
        
        self.rgb2feat = nn.ModuleList([
            RGB2FeatProjector1(4*nch, 1, stride=(1,1,1), act=actenc, acto=actenc, norm=norm, conv_mode=conv_mode),
            RGB2FeatProjector1(2*nch, 1, stride=(1,1,1), act=actenc, acto=actenc, norm=norm, conv_mode=conv_mode),
            RGB2FeatProjector1(nch, 1, stride=(1,1,1), act=actenc, acto=actenc, norm=norm, conv_mode=conv_mode)
        ])

        self.encoder_blocks = nn.ModuleList([
            EncoderBlock1(4*nch, 8*nch), # 0
            EncoderBlock1(2*nch, 4*nch), # 1
            EncoderBlock1(nch, 2*nch)    # 2
        ])
        ...
        assert len(self.encoder_blocks) == len(self.decoder_blocks)
    ...
    def forward(self, x, scale=None, fadein=0.0):
        scale = scale if scale is not None else len(self.encoder_blocks)

        # Encoder
        flow = list(range(scale))[:scale]
        flow.reverse()
        e = self.rgb2feat[flow[0]](x)
        for counter, i in enumerate(flow):
            e = self.encoder_blocks[i](e)
            if counter == 0 and fadein > 0 and scale > 1:
                _, _, t, h, w = e.shape 
                x_down = nn.functional.interpolate(x, size=[t,h,w], mode='trilinear', align_corners=False)
                e_prev = self.rgb2feat[flow[1]](x_down)
                e = e_prev * fadein + e * (1.0 - fadein)
