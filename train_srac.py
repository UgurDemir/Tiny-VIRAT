...
def train(gen, disc, neta, crit_ac, loader, epoch):
    ...
    for i, batch in enumerate(loader):
        ...
        if True:
            ...
            # Backward G
            gen.optimizer.zero_grad()
            g_loss, ginfo = gen.calc_loss(rec, gt, disc=disc, neta=neta, action_labels=label)
            g_loss.backward()
            gen.optimizer.step()
            # log
        ...
        if i % opt['image_freq'] == 0:
            ...
            ctx_vis = torch.nn.functional.interpolate(ctx[:,:,tid,:,:], size=[h, w], mode='bilinear', align_corners=False)
...
def validate(gen, disc, neta, crit_ac, loader, epoch):
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            acin = rec.detach()
            # Resnormalization
            if 'norm_to_01' in opt:
                acin = (acin + 1.0) * 0.5 if opt['norm_to_01'] else acin
            y = neta(acin)
            ac_loss = crit_ac(y, label)
            logmap['v_ac_loss'] = ac_loss.item()

            _, ginfo = gen.calc_loss(rec, gt, disc=disc, neta=neta, action_labels=label)
